...
if not all(type(item) is int for item in yes_by_int):
	report('Data type bug', 'yes_by_int should be a list of ints - first several items: ' + str(yes_by_int[:10]), 'No further help available')
	sys.exit(1)
...
try:
	...
	if not check:
		report('Value bug', 'Wrong value in yes_by_int - first several items: ' + str(yes_by_int[:10]), 'No further help available')
...
if not all(type(item) is int for item in no_by_int):
	report('Data type bug', 'no_by_int should be a list of ints - first several items: ' + str(no_by_int[:10]), 'No further help available')
	sys.exit(1)
...
try:
	...
	if not check:
		report('Value bug', 'Wrong value in no_by_int - first several items: ' + str(no_by_int[:10]), 'No further help available')
