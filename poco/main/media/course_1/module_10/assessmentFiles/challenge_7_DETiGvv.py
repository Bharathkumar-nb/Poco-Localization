...
try:
	...
	msg = str(e)
	if (not 'Unknown property area type:' in msg) or (not 'Desert' in msg):
		report('Function error', 'pa_converter did not do proper raise: ' + msg, 'No further help available')
except Exception as e:
	report('Generic error', 'On your own', e)
	sys.exit(1)
else:
	report('Function error', 'pa_converter did not do proper raise', 'Look for missing raise statement as last line of function')
