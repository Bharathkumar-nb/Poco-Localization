...
'''
float_all_fares = [person['Fare'] for person in titanic_starter_table]

sum_all_fares = sum(float_all_fares)

average_fare = sum_all_fares/len(float_all_fares)
'''
...
try:
        xfloat_all_fares = [person['Fare'] for person in titanic_starter_table]

        xsum_all_fares = sum(xfloat_all_fares)

	check = (average_fare == xsum_all_fares)/len(xfloat_all_fares)
except Exception as e:
	report('Generic error', 'On your own', e)
else:
	if not check:
