...
average_name_length = float(sum_all_names)/len(len_all_names)
'''
...
try:
	average_name_length		# does var exist?
except NameError as e:
	report('Name error', 'Typically a typo', e)
	sys.exit(1)

if not isinstance(average_name_length, float):
	report('Data type bug', 'average_name_length is not a float', 'No further help available')
	sys.exit(1)
...
try:
	...
	check = (average_name_length == float(xsum_all_names)/len(xlen_all_names))
except Exception as e:
	report('Generic error', 'On your own', e)
else:
	if not check:
		report('Value bug', 'Wrong value in average_name_length of ' + str(repr(average_name_length)), 'No further help available')
