...
answer = [1 if row['Loan_Status'] == 'Y' else 0 for row in loan_table]
...
if not all(type(item) == int for item in int_loan_status):
	report('Type bug', 'int_loan_status should contain only ints - first several rows are ' + str(repr(int_loan_status[:10])), 'No further help available')
	sys.exit(1)
...
try:
	...
	if not check:
		report('Value bug', 'Wrong value in int_loan_status: first several rows are ' + str(repr(int_loan_status[:10])), 'No further help available')
