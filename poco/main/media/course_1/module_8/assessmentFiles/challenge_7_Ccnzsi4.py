...
'''
wrangled_loan_column = [row if row['LoanAmount'] != '' else loan_amount_default for row in loan_table]
...
try:
	wrangled_loan_column		# does var exist?
except NameError as e:
	report('Name error', 'Typically a typo', e)
	sys.exit(1)

if not isinstance(wrangled_loan_column, list):
	report('Data type bug', 'wrangled_loan_column is not a list', 'No further help available')
	sys.exit(1)

x_wrangled_loan_column = [float(row['LoanAmount']) if row['LoanAmount'] != '' else loan_amount_default for row in xloan_table]
answer_len = len(x_wrangled_loan_column)

if len(wrangled_loan_column) != answer_len:
	report('Length bug', 'wrangled_loan_column should have length of ' + str(answer_len) + ' but has length ' + str(len(wrangled_loan_column)), 'No further help available')
	sys.exit(1)
...
try:
	check = (wrangled_loan_column == x_wrangled_loan_column)  # 
except Exception as e:
	report('Generic error', 'On your own', e)
else:
	if not check:
		report('Value bug', 'Wrong value in wrangled_loan_column. Here are first 10 items: ' + str(wrangled_loan_column[:10]), 'Remember to float string values')
