...
if not all(type(item) == int for item in int_loan_status):
	report('Type bug', 'int_loan_status should contain only ints - first several rows are ' + str(repl(int_loan_status[:10])), 'No further help available')
	sys.exit(1)
...
try:
	...
	if not check:
		report('Value bug', 'Wrong value in int_loan_status: first several rows are ' + str(repl(int_loan_status[:10])), 'No further help available')
