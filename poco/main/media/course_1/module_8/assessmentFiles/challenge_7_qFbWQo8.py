...
x_full_loan_column = [float(row['LoanAmount']) if row['LoanAmount'] != '' else loan_amount_default for row in xloan_table]
...
try:
	...
	if not check:
		report('Value bug', 'Wrong value in full_loan_column. Here are first 10 items: ' + str(full_loan_column[:10]), 'Remember to float string values')
