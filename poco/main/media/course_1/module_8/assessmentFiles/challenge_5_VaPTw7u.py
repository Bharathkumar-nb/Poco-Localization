...
try:
	...
	percent = sum(_x)/len(_x)
	check = (int(loan_amount_default) == int(percent))  # taking int to avoid precision mismatch - 146.412162162
except Exception as e:
	report('Generic error', 'On your own', e)
else:
	report('Debugging1', str(loan_amount_default), str(percent))
	if not check:
