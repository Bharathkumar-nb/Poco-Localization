...
loan_amount_mean = sum(amounts)/len(amounts)
'''
import sys
...
try:
	loan_amount_mean		# does var exist?
...
try:
	check = (loan_amount_mean == sum(xamounts)/len(xamounts))
except Exception as e:
	report('Generic error', 'On your own', e)
else:
	if not check:
		report('Value bug', 'Wrong value in loan_amount_mean of ' + str(repr(loan_amount_mean)), 'No further help available')
		sys.exit(1)