...
try:
	loan_ammount_mean		# does var exist?
...
try:
	check = (loan_ammount_mean == sum(xamounts)/len(xamounts))
except Exception as e:
	report('Generic error', 'On your own', e)
else:
	if not check:
		report('Value bug', 'Wrong value in loan_ammount_mean of ' + str(repr(loan_ammount_mean)), 'No further help available')
		sys.exit(1)