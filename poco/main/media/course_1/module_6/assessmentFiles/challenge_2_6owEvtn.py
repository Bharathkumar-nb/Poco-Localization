...
if len(expensive_table) != 2:
	report('Length bug', 'expensive_table should have length of 2 but has length ' + str(len(expensive_table)), 'Make sure to use float function')
	sys.exit(1)

try:
	check = (expensive_table == [row for row in titanic_starter_table if float(row['Fare'])>30])
except Exception as e:
	report('Generic error', 'On your own', e)
else:
	if not check:
