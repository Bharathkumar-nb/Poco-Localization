...
train_accuracy = (pp_count_train + nn_count_train)/float(train_table)

pp_count_test = test_cases.count(['1','1'])
nn_count_test = test_cases.count(['0','0'])

test_accuracy = (pp_count_test + nn_count_test)/float(test_table)

if 	train_accuracy > test_accuracy:
	report('Testing bug', 'Training accuracy ({1}) greater than test accuracy ({2}).'.format(train_accuracy, test_accuracy), 'No further help available')
	sys.exit(1)
